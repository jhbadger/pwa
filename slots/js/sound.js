// Synthesized sound effects for the slot machine — Web Audio only, no audio
...
// White noise through a bandpass filter reads as a mechanical click/clunk far
// better than any oscillator — real clicks are noise, not tone.
...
export function playLeverPull() {
  ...
  noiseBurst(t0, 0.12, { freq: 300, q: 0.6, gain: 0.35 });
  tone(180, t0, 0.15, { type: 'square', gain: 0.15 });
}

// Looping ratchet noise for the duration the reels are visibly spinning.
// Returns a stop() function — call it once the last reel lands.
...
export function playReelStop() {
  noiseBurst(ensureAudio().currentTime, 0.06, { freq: 1200, q: 2, gain: 0.3 });
}
...
export function playWin(multiplier) {
  const ctx = ensureAudio();
  const t0 = ctx.currentTime;
  const notes = multiplier >= 40 ? CHIME.length : multiplier >= 10 ? 4 : multiplier >= 3 ? 3 : 2;
